...
$sql = "select us_id,us_nm,us_account,base_amount/'{$unit}' as base_amount,(select wechat from us_base where us_id=a.us_id) as wechat from us_asset as a where `base_amount`/'{$unit}' >= 100 and us_id not in (select us_id from us_base where scale=1) order by base_amount desc";
$db->query($sql);
$one_rows = $db->fetchAll();
print_r($one_rows);die;
