...
{
    $db = new DB_COM();
    $sql = "SELECT g.id,g.name,g.us_id,g.is_del,g.is_flirt,g.is_audit,g.why,g.scale,g.group_manager_name,g.is_give_ccvt,g.group_type,g.is_admin_del,t.name as group_type_name FROM bot_group as g LEFT JOIN bot_group_type as t on g.group_type=t.id WHERE g.us_id = '{$us_id}' AND g.is_test=1 ORDER BY g.intime ASC ";
    $db -> query($sql);
    $row = $db -> fetchAll();
    return $row;
}
...
function glory_integral_list()
{
    $db = new DB_COM();
    $unit = get_la_base_unit();
    $sql = "SELECT a.credit_id,a.debit_id,a.tx_amount/'{$unit}' as tx_amount,a.ctime,a.utime,b.wechat as give_account,c.wechat as receive_account FROM us_glory_integral_change_log as a LEFT JOIN us_base as b ON a.credit_id=b.us_id LEFT JOIN us_base as c ON a.debit_id=c.us_id ORDER BY a.ctime DESC";
    $db -> query($sql);
    $row = $db -> fetchAll();
    return $row;
}
