...
{
    $db = new DB_COM();
    $sql = "SELECT g.*,t.name as group_type_name FROM bot_group as g LEFT JOIN bot_group_type as t on g.group_type=t.id WHERE g.us_id = '{$us_id}' AND g.is_test=1 ORDER BY g.intime ASC ";
    $db -> query($sql);
    $row = $db -> fetchAll();
    if ($row){
...
function save_group($date)
{
    ...
    $sql = "update bot_group set name = '{$date['name']}' , 
         ...
         uptime='{$time}',dis='{$date['dis']}',qr_code_address='{$date['qr_code_address']}',ranking_change_switch='{$date['ranking_change_switch']}',news_switch='{$date['news_switch']}',chat_time='{$date['chat_time']}',leave_message_switch='{$date['leave_message_switch']}' where id='{$date['group_id']}' ";
    $db->query($sql);
    if (!$db->affectedRows()){
        return false;
    }
