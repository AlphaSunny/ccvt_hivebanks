 ...
 * 荣耀积分排行
 */
function score_ranking($offset,$limit){

    $db = new DB_COM();
    $sql = "select a.base_amount,(SELECT wechat from us_base WHERE us_id=a.us_id) as wechat from us_asset as a WHERE a.asset_id='GLOP' ORDER BY a.base_amount DESC  limit $offset,$limit";
    $db->query($sql);
    $rows = $db->fetchAll();
    return $rows;
}