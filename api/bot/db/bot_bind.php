...
{
    $db = new DB_COM();
    $sql = "SELECT id,name,ba_id,is_del,is_flirt,us_id,is_test FROM bot_group WHERE is_del=1 AND is_audit=2 ORDER BY intime asc";
    $db -> query($sql);
    $rows = $db -> fetchAll();
    return $rows;
}
...
function get_qrcode($us_id)
{
    ...
    if ($row){
        ...
        if ($t){
            ...
            $sql = "select count(bot_message_id) as count from bot_message WHERE us_id='{$us_id}' bot_create_time>='{$t['login_in_time']}'";
            $db->query($sql);
