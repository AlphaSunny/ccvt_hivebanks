...
(function () {
    ...
    var ObjectContainerClass = InteractiveClass.extend({
        ...
        removeChild: function (child) {
            ...
            // 濡傛灉鏄敮鎾戞渶澶у楂樼殑child琚Щ闄や簡锛岄噸鏂板鐞嗘渶澶у楂�
            if (this.maxWidth == child.x + child.width) {
                this.maxWidth = 0;
        ...
        dispatchMouseEvent: function (type, x, y) {
            ...
            if (type != 'mousemove') {
                return; // 涓嶆槸 mousemove浜嬩欢鍒欏埌姝ょ粨鏉�
            }
            // 浠ヤ笅鏄鐞唌ousemove浜嬩欢
            for (var k=0; k<this.hoverChildren.length; k++) {
                // 鍘熸潵hoverChildren涓湁鐨勶紝鐜板湪娌℃湁鐨勶紝杞€屾墽琛� mouseout
                var has = false;
                ...
                if (!has) {
                    //涓嶅瓨鍦ㄤ簡锛屽鐞� this.hoverChildren[k] 鐨刴ouseout
                    // 鍒氬ソ鍙堟湁浜嬩欢鍦ㄧ洃鍚琺ouseout锛屽垯鎵ц
            ...
            // 鍘熸潵hoverChildren涓病鏈夌殑锛岀幇鍦ㄦ湁浜嗭紝璇佹槑mouseover
            for (var k=0; k<_hoverChildren.length; k++) {
                ...
                if (!has) {
                    //璇佹槑榧犳爣鍒氳繘鍏ワ紝澶勭悊mouseenter鎴杕ouseover
